...
-- Seleciona o ataque do menor para o maior.
SELECT nome, ataque, defesa, destreza
FROM Pokemons
ORDER BY ataque DESC, defesa DESC;
-- DESC é do maior para o menor.

-- Agrupa o tipo, fazendo a média do ataque.
SELECT tipo, AVG(ataque) AS media_ataque
FROM Pokemons
GROUP BY tipo
ORDER BY ataque DESC;

-- Agrupando o tipo, e fazendo com que a média seja maior que 70.
SELECT tipo, AVG(ataque) AS media_ataque
FROM Pokemons
GROUP BY tipo 
